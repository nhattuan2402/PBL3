<%@page import="database.MayBayDAO"%>
	...
	<div class="container-fluid">
		<form action="<%=url%>/khach-hang-controller" method="post"
			...
			<div class="row">
				...
				ChuyenBay chuyenbaydiduocchon = (ChuyenBay) session.getAttribute("chuyenBayDiDuocChon");
				String mamaybaydi = chuyenbaydiduocchon.getMaMayBay();
				MayBayDAO mbdao = new MayBayDAO();
				MayBay mbd = mbdao.selectByID(mamaybaydi);
				%>
				<div class="matran  col-xs-12  col-md-6" id="maybay__chuyendi">
					<div class="maTranViTriChoNgoi " id="maTranViTriChoNgoicd">
						<script>
							var maTranHtml = taoMaTranChoNgoichuyendiA32X(
						<%=mbd.getSoGheThuongGia()%>
							,
						<%=mbd.getSoCotThuongGia()%>
							,
						<%=mbd.getSoGhePhoThong1()%>
							,
						<%=mbd.getSoGhePhoThong2()%>
							,
						<%=mbd.getSoGhePhoThong3()%>
							,
						<%=mbd.getSoCotPhoThong()%>
							); //a321 (có ba bản) a320
						</script>
					</div>
				</div>
				...
				if (loaiVe.equals("Khứ hồi")) {
					...
					String mamaybayve = chuyenbaydiduocchon.getMaMayBay();
					MayBay mbv = mbdao.selectByID(mamaybayve);
				%>
				<div class="matran hidden col-xs-12  col-md-6" id="maybay__chuyenve">
					<div class="maTranViTriChoNgoi " id="maTranViTriChoNgoicv">
						<script>
							var maTranHtml = taoMaTranChoNgoichuyenveA32X(
						<%=mbv.getSoGheThuongGia()%>
							,
						<%=mbv.getSoCotThuongGia()%>
							,
						<%=mbv.getSoGhePhoThong1()%>
							,
						<%=mbv.getSoGhePhoThong2()%>
							,
						<%=mbv.getSoGhePhoThong3()%>
							,
						<%=mbv.getSoCotPhoThong()%>
							);
						</script>
					</div>
				</div>
